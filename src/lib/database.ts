import sqlite3 from 'sqlite3';
import { promisify } from 'util';
import path from 'path';

// Database connection setup
const dbPath = process.env.DATABASE_PATH || path.join(process.cwd(), 'lgpd_compliance.db');
const db = new sqlite3.Database(dbPath);

// Promisify database methods for async/await usage
const dbRun = promisify(db.run.bind(db));
const dbGet = promisify(db.get.bind(db));
const dbAll = promisify(db.all.bind(db));

// Types based on Gherkin scenarios
export type LGPDRequestType = 'ACCESS' | 'DELETION' | 'CORRECTION' | 'PORTABILITY';
export type LGPDRequestStatus = 'PENDING' | 'PROCESSING' | 'COMPLETED' | 'FAILED';

export interface LGPDRequest {
  id: string;
  user_id: string;
  company_id: string;
  type: LGPDRequestType;
  status: LGPDRequestStatus;
  reason: string;
  description: string;
  cpf_hash: string; // Hashed CPF for privacy
  pix_transaction_hash?: string;
  created_at: Date;
  response_due_at: Date;
  completed_at?: Date;
}

export interface EncryptedLGPDData {
  id: string;
  request_id: string;
  encrypted_blob: Buffer; // Sealed box encrypted data
  public_key_fingerprint: string; // Company's public key fingerprint
  created_at: Date;
}

export interface Company {
  id: string;
  name: string;
  public_key: string;
  created_at: Date;
  is_active: boolean;
}

/**
 * Initialize database tables according to requirements specification
 */
export async function initializeDatabase(): Promise<void> {
  try {
    // Companies table
    await dbRun(`
      CREATE TABLE IF NOT EXISTS companies (
        id TEXT PRIMARY KEY,
        name TEXT NOT NULL,
        public_key TEXT NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        is_active BOOLEAN DEFAULT 1
      )
    `);

    // LGPD requests table (metadata only - zero knowledge operator)
    await dbRun(`
      CREATE TABLE IF NOT EXISTS lgpd_requests (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
        company_id TEXT NOT NULL,
        type TEXT NOT NULL CHECK(type IN ('ACCESS', 'DELETION', 'CORRECTION', 'PORTABILITY')),
        status TEXT NOT NULL CHECK(status IN ('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED')),
        reason TEXT NOT NULL,
        description TEXT NOT NULL,
        cpf_hash TEXT NOT NULL,
        pix_transaction_hash TEXT,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        response_due_at DATETIME NOT NULL,
        completed_at DATETIME,
        FOREIGN KEY (company_id) REFERENCES companies (id)
      )
    `);

    // Encrypted data table (opaque blobs)
    await dbRun(`
      CREATE TABLE IF NOT EXISTS encrypted_lgpd_data (
        id TEXT PRIMARY KEY,
        request_id TEXT NOT NULL,
        encrypted_blob BLOB NOT NULL,
        public_key_fingerprint TEXT NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (request_id) REFERENCES lgpd_requests (id)
      )
    `);

    // Indexes for performance
    await dbRun('CREATE INDEX IF NOT EXISTS idx_requests_user_id ON lgpd_requests (user_id)');
    await dbRun('CREATE INDEX IF NOT EXISTS idx_requests_company_id ON lgpd_requests (company_id)');
    await dbRun('CREATE INDEX IF NOT EXISTS idx_requests_status ON lgpd_requests (status)');
    await dbRun('CREATE INDEX IF NOT EXISTS idx_encrypted_data_request_id ON encrypted_lgpd_data (request_id)');

    console.log('Database initialized successfully');
  } catch (error) {
    console.error('Database initialization failed:', error);
    throw error;
  }
}

/**
 * Create a new LGPD request
 */
export async function createLGPDRequest(request: Omit<LGPDRequest, 'id' | 'created_at' | 'response_due_at'>): Promise<string> {
  const id = `REQ-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  const created_at = new Date();
  const response_due_at = new Date(created_at.getTime() + (15 * 24 * 60 * 60 * 1000)); // 15 days from creation

  await dbRun(`
    INSERT INTO lgpd_requests (
      id, user_id, company_id, type, status, reason, description, 
      cpf_hash, pix_transaction_hash, created_at, response_due_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    id, request.user_id, request.company_id, request.type, request.status,
    request.reason, request.description, request.cpf_hash, request.pix_transaction_hash,
    created_at.toISOString(), response_due_at.toISOString()
  ]);

  return id;
}

/**
 * Store encrypted LGPD data (sealed box)
 */
export async function storeEncryptedLGPDData(
  requestId: string,
  encryptedBlob: Buffer,
  publicKeyFingerprint: string
): Promise<string> {
  const id = `ENC-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  
  await dbRun(`
    INSERT INTO encrypted_lgpd_data (id, request_id, encrypted_blob, public_key_fingerprint)
    VALUES (?, ?, ?, ?)
  `, [id, requestId, encryptedBlob, publicKeyFingerprint]);

  return id;
}

/**
 * Get LGPD requests for a user (returns metadata only)
 */
export async function getUserLGPDRequests(userId: string): Promise<LGPDRequest[]> {
  const rows = await dbAll(`
    SELECT * FROM lgpd_requests 
    WHERE user_id = ? 
    ORDER BY created_at DESC
  `, [userId]);

  return rows.map(row => ({
    ...row,
    created_at: new Date(row.created_at),
    response_due_at: new Date(row.response_due_at),
    completed_at: row.completed_at ? new Date(row.completed_at) : undefined
  }));
}

/**
 * Get LGPD requests for a company (returns metadata only)
 */
export async function getCompanyLGPDRequests(companyId: string): Promise<LGPDRequest[]> {
  const rows = await dbAll(`
    SELECT * FROM lgpd_requests 
    WHERE company_id = ? 
    ORDER BY created_at DESC
  `, [companyId]);

  return rows.map(row => ({
    ...row,
    created_at: new Date(row.created_at),
    response_due_at: new Date(row.response_due_at),
    completed_at: row.completed_at ? new Date(row.completed_at) : undefined
  }));
}

/**
 * Get encrypted data for a request (for company to decrypt)
 */
export async function getEncryptedLGPDData(requestId: string): Promise<EncryptedLGPDData | null> {
  const row = await dbGet(`
    SELECT * FROM encrypted_lgpd_data 
    WHERE request_id = ?
  `, [requestId]);

  if (!row) return null;

  return {
    ...row,
    created_at: new Date(row.created_at)
  };
}

/**
 * Update request status
 */
export async function updateRequestStatus(
  requestId: string, 
  status: LGPDRequestStatus, 
  completedAt?: Date
): Promise<void> {
  if (status === 'COMPLETED' && completedAt) {
    await dbRun(`
      UPDATE lgpd_requests 
      SET status = ?, completed_at = ? 
      WHERE id = ?
    `, [status, completedAt.toISOString(), requestId]);
  } else {
    await dbRun(`
      UPDATE lgpd_requests 
      SET status = ? 
      WHERE id = ?
    `, [status, requestId]);
  }
}

/**
 * Create a demo company for testing
 */
export async function createDemoCompany(): Promise<string> {
  const id = 'COMPANY-DEMO-TECHCORP';
  
  try {
    await dbRun(`
      INSERT OR REPLACE INTO companies (id, name, public_key, is_active)
      VALUES (?, ?, ?, ?)
    `, [id, 'TechCorp Ltd', 'DEMO_PUBLIC_KEY_TO_BE_REPLACED', true]);
    
    return id;
  } catch (error) {
    console.error('Failed to create demo company:', error);
    throw error;
  }
}

/**
 * Close database connection
 */
export function closeDatabase(): Promise<void> {
  return new Promise((resolve, reject) => {
    db.close((err) => {
      if (err) reject(err);
      else resolve();
    });
  });
}
